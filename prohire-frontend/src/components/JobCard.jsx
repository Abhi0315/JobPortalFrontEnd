...
const JobCard = ({ job }) => {
  ...
  useEffect(() => {
    const checkSavedStatus = () => {
      ...
      try {
        // Check if job is saved in localStorage first
        const savedJobs = JSON.parse(localStorage.getItem("savedJobs") || "[]");
        const isJobSaved = savedJobs.includes(job_id);
        setIsSaved(isJobSaved);

        // Then verify with API
        verifySavedStatusWithAPI(token);
      } catch (error) {
    ...
    const verifySavedStatusWithAPI = async (token) => {
      try {
        const response = await axios.get(
          `https://prohires.strangled.net/job/check_saved_job/${job_id}/`,
          {
            headers: { Authorization: `Token ${token}` },
          }
        );
        ...
        const savedJobs = JSON.parse(localStorage.getItem("savedJobs") || "[]");
        if (response.data.is_saved && !savedJobs.includes(job_id)) {
          savedJobs.push(job_id);
          localStorage.setItem("savedJobs", JSON.stringify(savedJobs));
        } else if (!response.data.is_saved && savedJobs.includes(job_id)) {
          const updatedSavedJobs = savedJobs.filter((id) => id !== job_id);
          localStorage.setItem("savedJobs", JSON.stringify(updatedSavedJobs));
        }
      } catch (error) {
        console.error("Error verifying saved status with API:", error);
      } finally {
        setIsCheckingSaved(false);
      }
    };
  ...
  const handleSaveJob = async (e) => {
    ...
    try {
      ...
      if (newSavedState) {
        ...
        const index = savedJobs.indexOf(job_id);
        if (index > -1) {
